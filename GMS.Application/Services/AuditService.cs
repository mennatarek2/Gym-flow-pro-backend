...
public class AuditService : IAuditService
{
    ...
    public async Task<Result<PagedResult<AuditEventDto>>> GetAuditEventsAsync(Guid tenantId, AuditEventQueryRequest query)
    {
        ...
        // Resolve actor display names for this page of results. ActorUserId is AppUser.Id (the
        // staff row's own primary key) - see ResolveActorUserIdAsync below and
        // AdminServiceStaffManagementTests.AuditActor_IsAppUserId_NotIdentityId, which pins this
        // contract deliberately (consistent with EntityId also being AppUser.Id for Staff audit
        // rows). A previous pass here had this backwards - joined against AppUser.UserId (the
        // ApplicationUser/Identity id) instead, which never matches AppUser.Id and made every
        // audit event fall back to "unknown actor"; that pass then "fixed" it by changing the
        // WRITE side to emit the identity id, which is what this existing test caught. Keeping
        // the write side alone and fixing this join instead is the correct, minimal fix - see
        // bug-166 in .wolf/buglog.json for the full corrected story.
        var actorIds = items
        ...
        if (actorIds.Count > 0)
        {
            ...
            var nameById = actorNames.ToDictionary(
                u => u.Id,
            ...
            foreach (var item in items)
            {
                if (item.ActorUserId.HasValue &&
                    nameById.TryGetValue(item.ActorUserId.Value, out var name) &&
